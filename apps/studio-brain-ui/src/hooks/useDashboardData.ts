...
export const browserHost = window.location.hostname || "localhost";
export const browserProtocol = window.location.protocol || "http:";
export const frontDoorUrl = `${browserProtocol}//${window.location.host}`;
const OPERATOR_PROTOCOL_URL = (port: number) => `http://${browserHost}:${port}`;

function isIpv4Address(value: string) {
  return /^\d{1,3}(\.\d{1,3}){3}$/.test(value);
}

export function frontDoorServiceUrl(service: "n8n" | "openclaw") {
  if (browserProtocol === "https:" && browserHost.includes(".") && !isIpv4Address(browserHost)) {
    return `https://${service}.${browserHost}`;
  }
  return service === "n8n" ? OPERATOR_PROTOCOL_URL(5678) : OPERATOR_PROTOCOL_URL(8100);
}
...
export const serviceProxyBase: Record<string, string> = {
...
export const serviceStatusApi: Record<string, string> = {
...
export const OPERATOR_NAME_KEY = "studioBrain.operatorName";
export const OPERATOR_TOKEN_KEY = "studioBrain.operatorToken";
export const CONCIERGE_TURNS_KEY = "studioBrain.conciergeTurns";

export const DEFAULT_CONCIERGE_TURNS: ConciergeTurn[] = [
  {
    role: "assistant",
...
export function loadStoredConciergeTurns(): ConciergeTurn[] {
  try {
...
export const serviceCatalog: Array<Omit<ServiceRecord, "state" | "detail">> = [
  {
    key: "project-state",
    name: "Project State",
    zone: "Control Plane",
    ...
    url: OPERATOR_PROTOCOL_URL(8080),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8090),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8100),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(5678),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(11434),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8130),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8140),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8110),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8120),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8150),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8160),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8170),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8180),
  ...
  },
  {
    ...
    url: OPERATOR_PROTOCOL_URL(8190),
...
export const supportSurface = [
  { name: "Caddy Edge", detail: "TLS and LAN front door for dashboard, n8n, and OpenClaw." },
  { name: "Postgres", detail: "Shared state, audit history, style records, and workflow persistence." },
  { name: "Shared Volumes", detail: "Projects, approvals, drafts, and delivery artifacts across services." },
];

export const primaryTabs: Array<{ id: TabId; label: string; summary: string; accent: string }> = [
  { id: "overview", label: "Overview", summary: "Platform posture, service health, and support fabric.", accent: "tab-overview" },
  { id: "operations", label: "Operations", summary: "Approvals, alerts, runtime recovery, and worker control.", accent: "tab-operations" },
  { id: "automation", label: "Automation", summary: "OpenClaw rules, starter packs, playbooks, and automation modules.", accent: "tab-automation" },
  { id: "context", label: "Context", summary: "Studio identity, style context, paths, and deployment posture.", accent: "tab-context" },
  { id: "settings", label: "Settings", summary: "First-run onboarding, integrations, alert destinations, and worker options.", accent: "tab-settings" },
];
...
export function defaultWorkspaceSettings(): WorkspaceSettings {
  return {
    ...
    style_seed: { name: "Default Studio Tone", raw_text: "", source_paths: [] },
    alert_destinations: { email_to: [], webhook_url: "" },
    integrations: { n8n: true, gmail_readonly: false, gmail_send: false, instagram: false, facebook: false },
    worker: {
      enabled: false,
    ...
    module_settings: {
      lead_intake: { enabled: true, minimum_fit_score: 55, response_sla_hours: 24, auto_create_projects: true },
      inbox_triage: { enabled: true, ignore_noise: true, high_priority_types: ["payment", "revision-request"] },
      content_pipeline: { enabled: true, default_platforms: ["instagram", "facebook"], require_assets: false, approval_required: true },
      audio_qc: { enabled: true, default_target: "streaming", hard_fail_on_clipping: true },
      session_prep: { enabled: true, filename_space_warning: true, remote_enabled: true },
      revision_parser: { enabled: true, default_daw: "reaper", confidence_threshold: 0.85 },
      delivery_packager: { enabled: true, require_qc_pass: true, include_manifest: true },
      mix_planner: { enabled: true, default_focus: ["vocals", "drums", "low-end translation"] },
    },
...
export function normalizeWorkspaceSettings(input?: Partial<WorkspaceSettings> | null): WorkspaceSettings {
  ...
  return {
    ...
    shared_paths: { ...defaults.shared_paths, ...sharedPaths },
    style_seed: { ...defaults.style_seed, ...styleSeed, source_paths: asArray(styleSeed.source_paths ?? defaults.style_seed.source_paths) },
    alert_destinations: {
...
export function summarizeTime(value: string) {
...
export function humanizeMissingField(field: string) {
...
export function fileLabel(path: string | null | undefined) {
...
export function buildDeliveryHistory(projectDetail: ProjectDetail | null) {
  if (!projectDetail) return [];
  return projectDetail.artifact_inventory
    .map((entry) => {
      const artifact = entry.artifact ?? {};
      const path = entry.artifact_path ?? (artifact.path as string | undefined) ?? (artifact.manifest_path as string | undefined) ?? (artifact.delivery_path as string | undefined);
      const kind = artifactKindLabel(artifact).toLowerCase();
      const label = `${kind} ${fileLabel(path)}`.toLowerCase();
      const deliveryLike = kind.includes("delivery") || kind.includes("manifest") || kind.includes("render") || label.includes("review_mix") || label.includes("instrumental") || label.includes("stems");
...
export function serviceStatusHighlights(payload: ServiceStatusPayload | null): Array<{ label: string; value: string }> {
  if (!payload) return [];
  return Object.entries(payload)
    .filter(([key]) => key !== "status" && key !== "settings")
    .slice(0, 6)
    .map(([key, value]) => ({ label: key.replace(/_/g, " "), value: formatStatusValue(value) }));
}

export function bootstrapStatusLabel(status: string) {
  return status === "skipped" ? "workflows present" : status.replace(/-/g, " ");
}

export function n8nWorkflowUrl(baseUrl: string, workflowSlug: string) {
  return `${baseUrl}/home/workflows?search=${encodeURIComponent(workflowSlug)}`;
}

export function studioVoicePreview(workspace: WorkspaceSettings, profile: StyleProfile | null) {
  ...
  if (preferredPhrases.length) return `Hi, this is ${studioName}. ${preferredPhrases.slice(0, 2).join(". ")}.`;
  if (toneMarkers.length) return `Hi, this is ${studioName}. We keep communication ${toneMarkers.slice(0, 3).join(", ")} while staying clear on next steps.`;
  if (workspace.style_seed.raw_text.trim()) return workspace.style_seed.raw_text.trim().slice(0, 180);
  return "Add a tone seed or rescan saved sources so the studio voice can be previewed here.";
}

export function serviceSettingsSummary(service: ServiceRecord, settings: ModuleSettings): string[] {
  switch (service.key) {
    case "lead-intake":
      return [`Fit floor ${settings.lead_intake.minimum_fit_score}`, `${settings.lead_intake.response_sla_hours}h reply SLA`, settings.lead_intake.auto_create_projects ? "auto-create projects" : "manual project creation"];
    case "inbox-triage":
      return [settings.inbox_triage.ignore_noise ? "noise filtering on" : "noise filtering off", `${settings.inbox_triage.high_priority_types.length} priority classes`];
    case "content-pipeline":
      return [settings.content_pipeline.approval_required ? "approval gate on" : "approval gate off", settings.content_pipeline.require_assets ? "assets required" : "asset-light drafts allowed", settings.content_pipeline.default_platforms.join(", ")];
    case "audio-qc":
      return [settings.audio_qc.default_target, settings.audio_qc.hard_fail_on_clipping ? "clip hard-fail" : "clip warning only"];
    case "session-prep":
      return [settings.session_prep.remote_enabled ? "remote prep allowed" : "local prep only", settings.session_prep.filename_space_warning ? "filename warnings on" : "filename warnings off"];
    case "revision-parser":
      return [settings.revision_parser.default_daw, `confidence ${settings.revision_parser.confidence_threshold}`];
    case "delivery-packager":
      return [settings.delivery_packager.require_qc_pass ? "QC pass required" : "QC optional", settings.delivery_packager.include_manifest ? "manifest included" : "manifest off"];
    case "mix-planner":
...
export function statusTone(state: ServiceState | string) {
  if (["healthy", "idle", "ready", "complete"].includes(state)) return "ok";
  if (["degraded", "busy", "queued", "claimed"].includes(state)) return "warn";
  return "bad";
}

export function primaryMode(workers: WorkerNode[]) {
  return workers.length ? "Mac mini + worker ready" : "single-machine default";
}

export function serviceLabel(service: ServiceRecord) {
...
async function fetchServiceState(path: string): Promise<{ state: ServiceState; detail: string }> {
  try {
    ...
    return { state: statusValue === undefined || statusValue === "ok" ? "healthy" : "degraded", detail: statusValue === "ok" || statusValue === undefined ? "reachable" : JSON.stringify(data) };
  } catch (error) {
    return { state: "offline", detail: error instanceof Error ? error.message : "unreachable" };
  }
}

export function groupByZone(services: ServiceRecord[]) {
...
export function serviceManagedIn(service: ServiceRecord): string {
...
export function servicePrimaryTab(service: ServiceRecord): TabId {
  if (service.key === "project-state" || service.key === "studio-worker") return "operations";
...
export function serviceDependencyHints(service: ServiceRecord): string[] {
  switch (service.key) {
    case "project-state":
    ...
    default:
      return service.zone === "Automation Modules" ? ["OpenClaw posture", "Ollama", "project-state approvals"] : ["project-state", "shared paths", "production pipeline"];
  }
}

export function serviceRecommendedAction(service: ServiceRecord): string {
...
export function workflowTone(state: "ready" | "watch" | "action"): "ok" | "warn" | "bad" {
  if (state === "ready") return "ok";
  if (state === "watch") return "warn";
  return "bad";
}

export async function loadDashboardData(auditDateFrom = "", auditDateTo = ""): Promise<DashboardData> {
  ...
  const [services, workers, rules, rulePacks, starterPacks, playbooks, tasks, approvals, jobHistory, projects, leads, auditLog, styleProfiles, alerts, runtimeAlerts, runtimeRecovery, bootstrapStatus, workspace] =
    await Promise.all([
      Promise.all(serviceCatalog.map(async (service) => ({ ...service, ...(await fetchServiceState(service.healthUrl)) }))),
  ...
  const [sessionManifestPreview, mixPlanPreview, renderPlanPreview, listeningReportPreview] = await Promise.all([
    ...
    fetchOptionalJson<MixPlanPreview>(`${API.studioWorker}/mix-plan/preview`, {
      ...
      body: JSON.stringify({
        ...
        session_manifest: { project_root: previewProjectRoot, stem_count: 0, reference_count: 0, readiness: { ready_for_planning: false } },
...
export function createInitialDashboardData(): DashboardData {
  return {
    refreshedAt: null,
    services: serviceCatalog.map((service) => ({ ...service, state: "degraded", detail: "loading" })),
    workers: [],
    rules: [],
    rulePacks: [],
    starterPacks: [],
    ...
    auditLog: [],
    styleProfiles: [],
    alerts: { configured_channel_count: 0, channels: [], thresholds: [] },
    runtimeAlerts: { approvals_waiting: 0, failed_worker_tasks: 0, claimed_worker_tasks: 0, expired_worker_leases: 0, stale_workers: [], active_alerts: [] },
    runtimeRecovery: {
      stale_workers: [],
      failed_tasks: [],
      claimed_tasks: [],
      summary: { failed_task_count: 0, claimed_task_count: 0, expired_claim_count: 0, stale_worker_count: 0 },
    },
    bootstrapStatus: { status: "pending", workflow_count: 0, detail: "Waiting for bootstrap status." },
    workspace: {
      connection_center: [],
      readiness_checks: [],
      readiness_summary: { ready_count: 0, partial_count: 0, needs_attention_count: 0, optional_count: 0 },
...
export function useDashboardData() {
  const [data, setData] = useState<DashboardData>(createInitialDashboardData());
