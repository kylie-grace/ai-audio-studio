...
def execute_package_delivery(payload: dict, settings) -> dict:
    ...
    delivery_root = translate_path(payload.get("delivery_path", settings.delivery_path), settings.path_translation_json) / project_slug / payload.get("package_name", "delivery-package")
    delivery_root.mkdir(parents=True, exist_ok=True)
    copied: list[str] = []
    for file_path in payload["file_paths"]:
        source = translate_path(file_path, settings.path_translation_json)
